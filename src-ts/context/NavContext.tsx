...
export const NavContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    navParent.addEventListener('click', (e) => {
      ...
      if (
        (!isNavExist && navParent.classList.contains('open')) ||
        e.target.closest('.mo h1') ||
        e.target.closest('.mo button')
      ) {
        setIsNavOpen(false);
      }
    });
  ...
  return (
    <NavContext.Provider
      value={{ navs, navRef, isNavOpen, setIsNavOpen, navMenu, setNavMenu }}
    >
      {children}
    </NavContext.Provider>
  );
